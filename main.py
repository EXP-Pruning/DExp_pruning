...
parser.add_argument(
    '--compress_rate',
    type=str,
    default=['0.4', '0.4', '0.52', '0.54', '0.55', '0.61', '0.64', '0.64', '0.79', '0.73', '0.66', '0.6', '0.6'],
    help='compress rate of each conv')
parser.add_argument(
    '--arch',
    type=str,
    default='vgg_16_bn',
    choices=('resnet_50','resnet_18','resnet_56','resnet_110','vgg_16_bn','googlenet'),
...
if args.dataset=='cifar10':
    ...
    trainloader_1 = torch.utils.data.DataLoader(trainset, batch_size=1, shuffle=True, num_workers=4,pin_memory=True)
    ...
    trainloader = torch.utils.data.DataLoader(trainset, batch_size=args.train_batch_size, shuffle=True, num_workers=0)
    trainloader_1 = torch.utils.data.DataLoader(trainset, batch_size=1, shuffle=True, num_workers=4)
...
def train_1(epoch, cov_id, arch,optimizer, scheduler):
    print_logger.info('\nEpoch: %d' % epoch)
    net.train()
    L= []
    train_loss = 0
    ...
    if args.arch == 'vgg_16_bn' :
        net.features.norm0.eval()
        net.features.norm1.eval()
        net.features.norm3.eval()
        net.features.norm4.eval()
        net.features.norm6.eval()
        net.features.norm7.eval()
        net.features.norm8.eval()
        net.features.norm10.eval()
        net.features.norm11.eval()
        net.features.norm12.eval()
        net.features.norm14.eval()
        net.features.norm15.eval()
        net.features.norm16.eval()
        net.classifier.norm1.eval()
    ...
    for batch_idx, (inputs, targets) in enumerate(trainloader_1):
        with torch.cuda.device(device):
            ...
            progress_bar(batch_idx,len(trainloader_1),
                         'Cov: %d | Loss: %.3f | Acc: %.3f%% (%d/%d)'
                         % (cov_id, train_loss / (batch_idx + 1), 100. * correct / total, correct, total))
    print_logger.info('L[0]:{:.2f},L[1]:{:.2f}'.format(L[0],L[1]))
    return L
...
def main():
    for cov_id in range(args.start_cov, len(convcfg)):
        ...
        if cov_id == 0:
            ...
            if args.arch=='resnet_50':
                ...
                if cov_id+1 not in skip_list:
                    ...
                    m.layer_mask(epoch, cov_id + 1, trainloader_1, resume=args.resume_mask,
                                 param_per_cov=param_per_cov_dic[args.arch], arch=args.arch)
                    for epoch in range(0, 1):
                        L = train_1(epoch, cov_id + 1, args.arch, optimizer, scheduler)
                        scheduler.step()
                    epoch = 1
                    m.layer_mask(epoch, cov_id + 1, trainloader_1, resume=args.resume_mask,
                                 param_per_cov=param_per_cov_dic[args.arch], arch=args.arch)
        ...
        optimizer = optim.SGD(net.parameters(), lr=0.0001, momentum=0.9, weight_decay=5e-4)
        scheduler = optim.lr_scheduler.MultiStepLR(optimizer, milestones=lr_decay_step, gamma=0.1)
        m.layer_mask(epoch, cov_id + 1,  trainloader_1, resume=args.resume_mask, param_per_cov=param_per_cov_dic[args.arch], arch=args.arch)
        for epoch in range(0,1) :
            L = train_1(epoch, cov_id + 1, args.arch,optimizer, scheduler)
            scheduler.step()
        ...
        if cov_id + 1 == len(convcfg):
            for epoch in range(args.epochs, args.epochs + 10):
                train(epoch, cov_id + 1, optimizer, scheduler)
                scheduler.step()
                test(epoch, cov_id + 1, optimizer, scheduler)
            m.pri_comp()
